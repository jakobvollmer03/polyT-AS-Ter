...
keep_unclassifiable_events <- if (!is.null(snakemake@params[["keep_unclassifiable_events"]])) {
  as.logical(snakemake@params[["keep_unclassifiable_events"]])
} else {
  FALSE
}
...
load_junctions <- function(junction_file, isoseq = FALSE) {
  ...
  # Handle case where column names might already be set
    ...
    if (ncol(junctions) >= 4) {
      ...
      } #else if ("majiq_dpsi") %in% colnames(junctions) {
        #junctions <- junctions %>%
        #  mutate(dpsi = as.numeric(majiq_dpsi))
      #}
  ...
  if (!"event_type" %in% colnames(junctions) && !"feature_type" %in% colnames(junctions)) {
    # for granges consensus. Not a permanent solution!
    if ("majiq_feature_type" %in% colnames(junctions)) {
      setnames(junctions, "majiq_feature_type", "event_type")
...
classify_junction <- function(junction, index, exons_gr, tolerance = 2) {
  ...
  if (is_prelabeled_ir) {
    ...
    # Check dpsi: if negative, the sequence is spliced out in test group → cIJ
  ...
  if (is_prelabeled_ir) {
    ...
    # Even if no exons found, still mark as IR or cIJ based on dpsi. Revisit when SpliCeAT has its own dPSI and event_types.
    if (!is.na(result$dpsi) && !result$dpsi > 0) {
      result$event_type <- "cIJ"
      result$confidence <- "pre-labeled_IR_dpsi<0"
    } else {
  ...
  if (nrow(js_matches) > 0 && nrow(je_matches) == 0) {
    ...
    if (length(overlapping_exons) > 0) {
      ES <- paste0("ES", length(unique(overlapping_exons)))
    } else {
      ES <- ""
    }
    ...
    if (!is.na(canon_je)) {
      if (length(overlapping_exons) == 0) {
        ...
        position_plus <- ifelse(je < canon_je, "us", "ds")
        position_minus <- ifelse(je > canon_je, "us", "ds")
      }
      ...
      if (length(overlapping_exons) > 0) {
        ...
        if (length(downstream_exon) > 0) {
          ...
          position_plus <- ifelse(je < ref_je, "us", "ds")
          position_minus <- ifelse(je > ref_je, "us", "ds")
        } else {
          ref_je <- NA_integer_
        }
      } else {
      ...
      active_position <- if (strand == "+") position_plus else position_minus
      ...
      if (strand == "+") {
        ...
        result$event_type <- paste0("cAA", position_plus, ES)
      } else {
        # Minus strand: je is donor
        result$event_type <- paste0("cAD", position_minus, ES)
      }
  ...
  if (nrow(js_matches) == 0 && nrow(je_matches) > 0) {
    ...
    if (!is.na(canon_js)) {
      if (length(overlapping_exons) == 0) {
      # Determine position relative to canonical
      position_plus <- ifelse(js < canon_js, "us", "ds")
      position_minus <- ifelse(js > canon_js, "us", "ds")
      }
      # Determine position relative to canonical based on skipped exons if present
      if (length(overlapping_exons) > 0) {
        position_plus <- ifelse(js < ref_js, "us", "ds")
        position_minus <- ifelse(js > ref_js, "us", "ds")
      }
      active_position <- if (strand == "+") position_plus else position_minus
      if (is.na(active_position)) {
        result$event_type <- "novel_junction"
      ...
      if (strand == "+") {
        ...
        result$event_type <- paste0("cAD", position_plus, ES)
      } else {
        # Minus strand: js is acceptor
        result$event_type <- paste0("cAA", position_minus, ES)
      }
    ...
    if (length(common_genes) > 0) {
      ...
      if (js_within_tolerance || je_within_tolerance) {
        if (js_within_tolerance && je_within_tolerance) {
          ...
          result$event_type <- ifelse(je < canon_je, "cAAus", "cAAds")
          result$gene_id <- common_genes[1]
          result$ref_js <- canon_js
          result$ref_je <- canon_je
          result$confidence <- "2"
          ...
          result$event_type <- ifelse(js < canon_js, "cADus", "cADds")
          result$gene_id <- common_genes[1]
          result$ref_js <- canon_js
          result$ref_je <- canon_je
          result$confidence <- "2"
      ...
      if (length(common_genes) > 0) {
        gene_id <- common_genes[1]
        ifelse(strand == "+",
               result$event_type <- paste0("cC", position_js, position_je, ES),
               result$event_type <- paste0("cC", position_je, position_js, ES))
        result$gene_id <- gene_id
...
#' Detect cryptic exon inclusion (cEI) pattern in clusters
#' 
#' cEI is detected when a cluster has 3+ junctions where some junctions
#' don't overlap but are connected through intermediate junctions.
#' Example: junctions at 1000-1200, 1000-2000, 1300-2000
#' (first and third don't overlap but both overlap with second)
#'  
#' @param cluster_df Data frame with junctions in a cluster (js, je columns required)
#' @return TRUE if cEI pattern detected, FALSE otherwise
detect_cEI_in_cluster <- function(cluster_df) {
  if (nrow(cluster_df) < 3) return(FALSE)
 
  js <- cluster_df$js
  je <- cluster_df$je
 
  positive_strand <- cluster_df$strand[1] == "+"
  up <- function(idx) if (positive_strand) js[idx] else je[idx]
  dn <- function(idx) if (positive_strand) je[idx] else js[idx]
 
  overlaps <- function(p, q) js[p] <= je[q] && js[q] <= je[p]
 
  k_idx  <- which(cluster_df$event_type == "canonical_discarded")
  ij_idx <- which(cluster_df$event_type != "canonical_discarded")
 
  if (length(k_idx) == 0 || length(ij_idx) < 2) return(FALSE)
 
  n_ij <- length(ij_idx)
  for (ii in 1:(n_ij - 1)) {
    for (jj in (ii + 1):n_ij) {
      i <- ij_idx[ii]; j <- ij_idx[jj]
      if (overlaps(i, j)) next  # i and j must not overlap
 
      for (k in k_idx) {
        # k upstream matches i, k downstream matches j
        if (up(k) == up(i) && dn(k) == dn(j)) return(TRUE)
        # or roles swapped: k upstream matches j, k downstream matches i
        if (up(k) == up(j) && dn(k) == dn(i)) return(TRUE)
      }
    }
  }
...
#' Cluster all junctions based on overlap and add a representative ID for each cluster
#' Also calculates cEI (cryptic exon inclusion) flag
#' 
#' @param results_df Data frame with junction classifications (must have chr, js, je, strand columns)
#' @param tolerance Maximum gap size to group junctions into clusters
#' @return Data frame with added cluster_id and cEI columns
#' @return results_df with cluster_id (integer) and cEI (logical) columns added
cluster_junctions_and_calculate_cEI <- function(results_df, tolerance = 2) {
 
  # ── initialise output columns ──────────────────────────────────────────────
  results_df$cluster_id <- NA_integer_
  results_df$cEI        <- FALSE
 
  # ── tag every row with its ORIGINAL position before splitting ─────────────
  # This is the key fix: orig_row travels with each sub-group so we always
  # know which rows of results_df to update.
  results_df$orig_row <- seq_len(nrow(results_df))
 
  # ── split by chromosome + strand ──────────────────────────────────────────
  chr_strand_groups <- results_df %>%
    group_by(chr, strand) %>%
    group_split()
 
  cluster_counter <- 1L
 
  for (group in chr_strand_groups) {
    if (nrow(group) == 0) next
 
    # Build GRanges; store orig_row as metadata so findOverlaps can recover it
    group_gr <- GRanges(
      seqnames = group$chr,
      ranges   = IRanges(start = group$js, end = group$je),
      strand   = group$strand,
      orig_row = group$orig_row          # ← original results_df row numbers
    )
 
    # Merge overlapping / near-adjacent junctions into cluster ranges.
    # min.gapwidth = tolerance + 1 means a gap of exactly `tolerance` bases
    # is still considered part of the same cluster.
    clusters <- GenomicRanges::reduce(group_gr, min.gapwidth = tolerance + 1,
                                      ignore.strand = FALSE)
 
    for (i in seq_along(clusters)) {
      hits          <- findOverlaps(group_gr, clusters[i], type = "any")
      orig_rows_hit <- mcols(group_gr)$orig_row[queryHits(hits)]
 
      # Write cluster ID back to the correct rows of results_df
      results_df$cluster_id[orig_rows_hit] <- cluster_counter
 
      # Subset the group to just these junctions and test for cEI
      cluster_subset <- group[group$orig_row %in% orig_rows_hit, ]
      if (detect_cEI_in_cluster(cluster_subset)) {
        results_df$cEI[orig_rows_hit] <- TRUE
      }
...
classify_all_junctions <- function(junction_file, gtf_file, output_file,
  ...
  cat(sprintf("Pre-labeled exonic contigs: %d\n", total_prelabeled_es))
  ...
  # Cluster junctions and calculate cEI
  cat("\nClustering junctions and detecting cryptic exon inclusion...\n")
  results_df <- cluster_junctions_and_calculate_cEI(results_df, tolerance = tolerance)
  
  n_clusters <- length(unique(results_df$cluster_id[!is.na(results_df$cluster_id)]))
  n_cEI <- sum(results_df$cEI == TRUE, na.rm = TRUE)
  cat(sprintf("Assigned %d junctions to %d clusters\n", nrow(results_df), n_clusters))
  cat(sprintf("Detected %d junction(s) with cryptic exon inclusion (cEI=TRUE)\n", n_cEI))
  ...
  if (n_canonical > 0) {
    cat(sprintf("\nDiscarding %d canonical junction(s) (both boundaries match annotated exon boundaries).\n",
                n_canonical))
    results_df <- results_df[!(results_df$event_type == "canonical_discarded"), ]
  }
  n_novel <- sum(results_df$event_type == "novel_junction", na.rm = TRUE)

  if (n_novel > 0) {
    cat(sprintf("\nIdentified %d novel junction(s) with no nearby annotation.\n", n_novel))

    if (is.null(keep_unclassifiable_events) || keep_unclassifiable_events == FALSE) {
      results_df <- results_df[!(results_df$event_type == "novel_junction"), ]
      cat(sprintf("Discarded novel junctions. %d junction(s) remain.\n", nrow(results_df)))
    } else {
      cat("Keeping unclassifiable events in output with event_type = novel_junction.\n")
    }
  }
  ...
  cat("\n=== Clustering Summary ===\n")
  cat(sprintf("Junctions with cEI detected:\n"))
  print(table(results_df$cEI))
